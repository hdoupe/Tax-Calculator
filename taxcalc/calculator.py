...
class Calculator():
    ...
    def _taxinc_to_amt(self):
        ...
        """
        TaxInc(self.__policy, self.__records).compute()
        SchXYZTax(self.__policy, self.__records).compute()
        GainsTax(self.__policy, self.__records).compute()
        AGIsurtax(self.__policy, self.__records).compute()
        NetInvIncTax(self.__policy, self.__records).compute()
        AMT(self.__policy, self.__records).compute()
    ...
    def _calc_one_year(self, zero_out_calc_vars=False):
        ...
        DependentCare(self.__policy, self.__records).compute()
        Adj(self.__policy, self.__records).compute()
        ALD_InvInc_ec_base(self.__policy, self.__records).compute()
        CapGains(self.__policy, self.__records).compute()
        SSBenefits(self.__policy, self.__records).compute()
        UBI(self.__policy, self.__records).compute()
        AGI(self.__policy, self.__records).compute()
        ItemDedCap(self.__policy, self.__records).compute()
        ItemDed(self.__policy, self.__records).compute()
        AdditionalMedicareTax(self.__policy, self.__records).compute()
        StdDed(self.__policy, self.__records).compute()
        ...
        self._taxinc_to_amt()
        F2441(self.__policy, self.__records).compute()
        EITC(self.__policy, self.__records).compute()
        RefundablePayrollTaxCredit(self.__policy, self.__records).compute()
        PersonalTaxCredit(self.__policy, self.__records).compute()
        AmOppCreditParts(self.__policy, self.__records).compute()
        SchR(self.__policy, self.__records).compute()
        EducationTaxCredit(self.__policy, self.__records).compute()
        CharityCredit(self.__policy, self.__records).compute()
        ChildDepTaxCredit(self.__policy, self.__records).compute()
        NonrefundableCredits(self.__policy, self.__records).compute()
        AdditionalCTC(self.__policy, self.__records).compute()
        C1040(self.__policy, self.__records).compute()
        CTC_new(self.__policy, self.__records).compute()
        IITAX(self.__policy, self.__records).compute()
