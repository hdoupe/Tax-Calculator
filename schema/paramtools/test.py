...
def test_range_validation(schema_def_path, base_spec_path):
    reform = {"_II_em": [{"year": 2020, "value": 5000}]}
    sb = SchemaBuilder(schema_def_path, base_spec_path)
    sb.build_schemas()
...
def test_range_validation_fail(schema_def_path, base_spec_path):
    reform = {"_II_em": [{"year": 2020, "value": -1}]}
    sb = SchemaBuilder(schema_def_path, base_spec_path)
    sb.build_schemas()
    with pytest.raises(exceptions.ValidationError) as excinfo:
        sb.load_params(reform)
...
def test_range_validation_on_named_variable_fails(
    schema_def_path, base_spec_path
):
    reform = {
...
def test_range_validation_on_default_variable(schema_def_path, base_spec_path):
    reform = {"_STD": [{"year": 2018, "MARS": "separate", "value": 12001.00}]}
    sb = SchemaBuilder(schema_def_path, base_spec_path)
    sb.build_schemas()
    sb.load_params(reform)
...
def test_range_validation_on_default_variable_fails(
    ...
    reform = {"_STD": [{"year": 2018, "MARS": "separate", "value": 11999.00}]}
    sb = SchemaBuilder(schema_def_path, base_spec_path)
    sb.build_schemas()
    with pytest.raises(exceptions.ValidationError) as excinfo:
        sb.load_params(reform)
